...
//                             $this_id=$response[$i];
//                             echo "act".$this_id;
$studentResultQuery="SELECT a.activity_id, a.activity_name ,ts.start_time,ts.end_time,ss.hours_granted, s.student_id ,s.checked_in ,s.first_name ,s.last_name, ss.attended FROM student_shifts ss, students s, activity a,time_slots ts  WHERE a.slot_id=ts.slot_id and ts.start_time>='".$range_start_time."' and ts.start_time<='".$range_end_time."' and a.slot_id = ts.slot_id AND ss.student_id = s.student_id AND ss.activity_id = a.activity_id ORDER BY ts.start_time;";
                                $studentResult = $db->query($studentResultQuery);
